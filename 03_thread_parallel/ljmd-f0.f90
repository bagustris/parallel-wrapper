...
!
! optimized f95 version using O(N**2) algorithm w/o newton's 3rd law.
!
...
!$OMP parallel DO default(SHARED) reduction(+:epot) &
!$OMP private(i,pos1,j,delta,rsq,rinv,r6,ffac)
  DO i=1, natoms
     pos1 = pos(i,:)
...
!        delta = delta - box*(ANINT(delta/box))
        ...
        IF (rsq < rcutsq) THEN
           ...
           epot = epot + 0.5_dbl*r6*(c12*r6 - c6)
           frc(i,1) = frc(i,1) + delta(1)*ffac
           frc(i,2) = frc(i,2) + delta(2)*ffac
           frc(i,3) = frc(i,3) + delta(3)*ffac
        END IF
     END DO
  END DO
...
PROGRAM LJMD
  ...
  ALLOCATE(pos(natoms,3),vel(natoms,3),frc(natoms,3))
