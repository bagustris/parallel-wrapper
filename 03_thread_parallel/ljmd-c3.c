    ...
    for(i=0; i < (sys->natoms)-1; ++i) {
        ...
        for(j=i+1; j < (sys->natoms); ++j) {
            ...
            if (rsq < rcsq) {
                ...
                /* no two threads must update the forces at 
                   the same time, or we get data corruption.
                   using omp atomic incurs much less overhead
                   than omp critical. may be ignored, though. */
#if defined(_OPENMP)
#pragma omp atomic
#endif
                sys->fx[i] += rx*ffac;
