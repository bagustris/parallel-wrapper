...
!
! optimized hybrid MPI/OpenMP parallel f95 version using O(N**2) algorithm and newton's 3rd law.
!
...
SUBROUTINE force
  ...
  INTEGER :: i, j, ii, tid, fromidx, toidx, ierror
  INTEGER, EXTERNAL :: omp_get_thread_num
 
  ep=0.0_dbl
  CALL MPI_BCAST(pos,3*natoms, MPI_REAL8, 0, mpicomm, ierror)

  !$OMP parallel default(SHARED) reduction(+:ep) &
  ...
  DO ii=0, natoms-1, nthreads*mpisize
     i = ii + nthreads*mpirank + tid
     IF (i > natoms-1) EXIT
     pos1 = pos(i,:)
     ...
     DO j=i+1, natoms
        ...
        IF (rsq < rcutsq) THEN
           ...
           cbuf(i,:,tid) = cbuf(i,:,tid) + delta*ffac
           cbuf(j,:,tid) = cbuf(j,:,tid) - delta*ffac
        END IF
     END DO
  END DO
...
PROGRAM LJMD
  ...
  IF (mpirank == 0) THEN
     WRITE(stdout,'(A,I2,A,I2,A)') 'running in parallel mode with ', mpisize,&
          ' MPI task(s) and ', nthreads, ' OpenMP thread(s)'
  ...
  ALLOCATE(pos(natoms,3),vel(natoms,3),frc(natoms,3),cbuf(natoms,3,nthreads))
