...
!
! optimized parallel f95 version using cell lists
!
...
MODULE cell
  ...
  REAL(KIND=dbl) :: dcell
  PRIVATE
  PUBLIC :: ncell, npair, mkcell, updcell, rmcell, clist, plist, npercell
...
SUBROUTINE force
  ...
  INTEGER :: i, j, k, n, m, ii, jj, kk, ierror
     ...
     DO j=1,npercell(i)-1
        ...
        DO k=j+1,npercell(i)
           ...
           IF (rsq < rcutsq) THEN
              ...
              cbuf(ii,:) = cbuf(ii,:) + delta*ffac
              cbuf(jj,:) = cbuf(jj,:) - delta*ffac
           END IF
        END DO
     END DO
  END DO
